...
use work.pwm_ctrl_v1_pkg.all;
use work.reg_utils.all;

entity pwm_ctrl_v1 is
    port (
        clk : in std_logic;
        rst : in std_logic;
        ...
        hwif_out : out pwm_ctrl_v1_out_t
    );
end entity pwm_ctrl_v1;

architecture rtl of pwm_ctrl_v1 is
    ----------------------------------------------------------------------------
    ...
    signal decoded_addr : std_logic_vector(3 downto 0);
    ...
    type \pwm_ctrl_v1.CTRL.ENABLE_combo_t\ is record
        next_q : std_logic;
        load_next : std_logic;
    end record;

    type \pwm_ctrl_v1.CTRL_combo_t\ is record
        ENABLE : \pwm_ctrl_v1.CTRL.ENABLE_combo_t\;
    end record;

    type \pwm_ctrl_v1.DUTY.VALUE_combo_t\ is record
        next_q : std_logic_vector(9 downto 0);
        load_next : std_logic;
    end record;

    type \pwm_ctrl_v1.DUTY_combo_t\ is record
        VALUE : \pwm_ctrl_v1.DUTY.VALUE_combo_t\;
    end record;
    type \pwm_ctrl_v1.DUTY_combo_t_array\ is array(natural range <>) of \pwm_ctrl_v1.DUTY_combo_t\;

    type field_combo_t is record
        CTRL : \pwm_ctrl_v1.CTRL_combo_t\;
        DUTY : \pwm_ctrl_v1.DUTY_combo_t_array\(0 to 1);
    end record;
    signal field_combo : field_combo_t;

    -- Field Storage Signals
    type \pwm_ctrl_v1.CTRL.ENABLE_storage_t\ is record
        value : std_logic;
    end record;

    type \pwm_ctrl_v1.CTRL_storage_t\ is record
        ENABLE : \pwm_ctrl_v1.CTRL.ENABLE_storage_t\;
    end record;

    type \pwm_ctrl_v1.DUTY.VALUE_storage_t\ is record
        value : std_logic_vector(9 downto 0);
    end record;

    type \pwm_ctrl_v1.DUTY_storage_t\ is record
        VALUE : \pwm_ctrl_v1.DUTY.VALUE_storage_t\;
    end record;
    type \pwm_ctrl_v1.DUTY_storage_t_array\ is array(natural range <>) of \pwm_ctrl_v1.DUTY_storage_t\;

    type field_storage_t is record
        CTRL : \pwm_ctrl_v1.CTRL_storage_t\;
        DUTY : \pwm_ctrl_v1.DUTY_storage_t_array\(0 to 1);
    end record;
    ...
    -- Readback Signals
    ----------------------------------------------------------------------------
    signal rd_mux_addr : unsigned(3 downto 0);
...
begin
    ...
    gen_field_logic_DUTY_dim0: for i0 in 0 to 1 generate
        ...
        -- Field: pwm_ctrl_v1.DUTY[].VALUE
        process(all)
            variable next_c: std_logic_vector(9 downto 0);
            variable load_next_c: std_logic;
        begin
            next_c := field_storage.DUTY(i0).VALUE.value;
            load_next_c := '0';
            if decoded_reg_strb.DUTY(i0) and decoded_req_is_wr then -- SW write
                next_c := (field_storage.DUTY(i0).VALUE.value and not decoded_wr_biten(9 downto 0)) or (decoded_wr_data(9 downto 0) and decoded_wr_biten(9 downto 0));
                load_next_c := '1';
            end if;
        ...
        process(clk) begin
            if false then -- async reset
                field_storage.DUTY(i0).VALUE.value <= 10x"0";
            elsif rising_edge(clk) then
                if rst then -- sync reset
                    field_storage.DUTY(i0).VALUE.value <= 10x"0";
                else
    ...
    begin
        ...
        if rd_mux_addr = 16#0# then
            readback_data_var(7 downto 0) := 8x"2";
            readback_data_var(15 downto 8) := 8x"1";
        ...
        for i0 in 0 to 1 loop
            if rd_mux_addr = 16#8# + i0 * 16#4# then
                readback_data_var(9 downto 0) := field_storage.DUTY(i0).VALUE.value;
            end if;
        end loop;
