...
| `drivers/pwm_ctrl/generated/` | committed PeakRDL output — run `regenerate.py`, never hand-edit |
| `drivers/pwm_ctrl/rdl/` | the SystemRDL register definition of record |
...
- **`drivers/pwm_ctrl/generated/` is generated and committed.** Edit
  `rdl/pwm_ctrl.rdl`, then `python regenerate.py`; never hand-edit the output.
  `regenerate.py` owns the exporter flags so CI and humans cannot diverge. Two
  checks catch a stale tree: the ctest from `peakrdl_check_generated()`, and
  the `.rdl` sha256 in `ipman-driver.json`.
- **The generated C header needs `peakrdl_compat.hpp` first on MSVC.** PeakRDL
  emits `__attribute__((__packed__))`, which MSVC rejects. The shim is safe
  only because every member of this map is a `uint32_t`; a map with sub-word
  members would need the real packing, and the generated `static_assert` on
  struct size is what would catch it.
